...
func testUserToManyAuthoredVideos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.AuthoredVideos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAuthoredVideos(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthoredVideos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AuthoredVideos = nil
	if err = a.L.LoadAuthoredVideos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthoredVideos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAuthoredVideos(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthoredVideos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AuthoredVideos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AuthoredVideos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AuthoredVideos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpAuthoredVideos(t *testing.T) {
	var err error
	...
	err = a.SetAuthoredVideos(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAuthoredVideos(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.AuthoredVideos[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.AuthoredVideos[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpAuthoredVideos(t *testing.T) {
	var err error
	...
	err = a.AddAuthoredVideos(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAuthoredVideos(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.AuthoredVideos) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.AuthoredVideos[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.AuthoredVideos[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Login`: `character varying`}
	_           = bytes.MinRead
)
