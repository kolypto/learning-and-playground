...
func testArticleToManyTagBtags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, btagDBTypes, false, btagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, btagDBTypes, false, btagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	_, err = tx.Exec("insert into \"barticle_tags\" (\"article_id\", \"tag_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"barticle_tags\" (\"article_id\", \"tag_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.TagBtags().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadTagBtags(ctx, tx, false, (*[]*Article)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TagBtags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TagBtags = nil
	if err = a.L.LoadTagBtags(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TagBtags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testArticleToManyAddOpTagBtags(t *testing.T) {
	var err error
	...
	foreigners := []*Btag{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, btagDBTypes, false, strmangle.SetComplement(btagPrimaryKeyColumns, btagColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Btag{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddTagBtags(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if first.R.ArticleBarticles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
		if second.R.ArticleBarticles[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}

		if a.R.TagBtags[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TagBtags[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TagBtags().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testArticleToManySetOpTagBtags(t *testing.T) {
	var err error
	...
	err = a.SetTagBtags(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetTagBtags(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	//
	// if len(b.R.ArticleBarticles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	// if len(c.R.ArticleBarticles) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	if d.R.ArticleBarticles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
	if e.R.ArticleBarticles[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
...
func testArticleToManyRemoveOpTagBtags(t *testing.T) {
	var err error
	...
	err = a.AddTagBtags(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveTagBtags(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	articleDBTypes = map[string]string{`ID`: `bigint`, `AuthorID`: `bigint`, `Title`: `text`, `Body`: `text`}
	_              = bytes.MinRead
)
